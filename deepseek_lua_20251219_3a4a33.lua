            ...
            if not Trophy:FindFirstChild("TouchInterest") then
                ...
                Trophy.Touched:Connect(function(hit)
                    ...
                    if player and player == game.Players.LocalPlayer and AutoWinsToggle then
                        ...
                        game:GetService("ReplicatedStorage").Events.CollectTrophy:FireServer()
                        Rayfield:Notify({
                            Title = "Win Collected!",
...
-- Laser Aim Section
local LaserSection = MainTab:CreateSection("Laser Aim")

local LaserBeam = Instance.new("Part")
LaserBeam.Name = "LaserBeam"
LaserBeam.Size = Vector3.new(0.1, 0.1, 100)
...
-- Laser Toggle
local LaserToggle = MainTab:CreateToggle({
    Name = "Show Laser",
    CurrentValue = false,
    Flag = "LaserToggle",
    Callback = function(Value)
                    ...
                    if character then
                        ...
                        if tool then
                            ...
                            if handle then
                                ...
                                local direction = ray.Direction.Unit * LaserDistance
                                
                                -- Position laser
                                LaserBeam.CFrame = CFrame.new(handle.Position, handle.Position + direction)
...
MainTab:CreateSlider({
    ...
    Callback = function(Value)
        LaserDistance = Value
        LaserBeam.Size = Vector3.new(0.1, 0.1, Value)
    end
})
...
local ESPTracerPosition = "Bottom" -- Bottom, Middle, Top
        ...
        if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if instances.Billboard then
                ...
                if ESPNames and ESPEnabled then
                    ...
                    instances.Billboard.ESP_Text.Text = string.format("%s [%d]", player.Name, math.floor(distance))
                end
            end
                ...
                if ESPTracers and ESPEnabled then
                    ...
                    if onScreen then
                        ...
                        if localCharacter and localCharacter:FindFirstChild("HumanoidRootPart") then
                            ...
                            local localScreenPos = camera:WorldToViewportPoint(localPos)
                            
                            instances.Tracer.From = Vector2.new(localScreenPos.X, localScreenPos.Y)
                            
                            if ESPTracerPosition == "Bottom" then
                                instances.Tracer.To = Vector2.new(screenPos.X, screenPos.Y + 5)
...
-- Teleport Tool (like Infinite Yield)
ToolsTab:CreateButton({
    Name = "Teleport Tool",
    Callback = function()
        Rayfield:Notify({
            Title = "Teleport Tool",
            Content = "Click anywhere to teleport (Ctrl+Click)",
            Duration = 5,
            Image = 4483362458
        })
...
-- Combined Fullbright + NoFog + FPS Boost
MiscTab:CreateButton({
